...
def setup_grid(grid_params):
    ...
    # layer interfaces
    zeta = np.arange(0.0, nz1 * dzeta, dzeta, dtype=float_wp)
    dz = np.zeros(nz, dtype=float_wp)
    z = np.zeros(nz1, dtype=float_wp)
    dz[0] = z1
    for k in range(1, nz1):
        z[k] = float_wp(1.0) / b * z1 * (np.exp(zeta[k]) - 1)
    for k in range(1, nz):
        dz[k] = z[k + 1] - z[k]
    ...
    for k in range(0, nz):
        z_m[k] = (
            ...
            / b
            * z1
            * (np.exp((k + float_wp(0.5)) * dzeta) - float_wp(1.0))
        )
...
def calc_jacobian(z_array, grid_params):
    ...
    jacobian = float_wp(1.0) / (z_array + z1 / b)
    if b < float_wp(1.0e-3):
        jacobian[:] = float_wp(1.0)

...
def update_euler(k, d, qground, w_vol, dz, dz_h, dt):
    ...
    flux_sum[j] = ( d[jp1] * ( w_vol[jp1] - w_vol[j] ) / dz_h[jp1]  \
                      - d[j] * ( w_vol[j] - w_vol[jm1] ) /  dz_h[j]  ) / dz[j] \
                - (k[jp1] - k[j]) / dz[j] - qground[j]

    flux_sum[0] =  d[1] * ( w_vol[1] - w_vol[0] ) / (dz_h[1]*dz[0]) \
                  - (k[1] - k[0]) / dz[0] - qground[0]
    flux_sum[nz-1] = - d[nz-1] * ( w_vol[nz-1] - w_vol[nz-2] ) / (dz_h[nz-1]*dz[nz-1]) \
                  - (k[nz-1] - k[nz-2]) / dz[nz-1] - qground[nz-1]
...
def update_implicit_tr(dt, dzeta, j_h, j_mid, d, k, w_vol, qground_tr, w_vol_new):
    ...
    #diagonal and off-diag vectors
    diag_main = float_wp(1.)/j_mid[j] + a[j]
    diag_lower = c[(1):(nz)]
    diag_upper = b[:(nz-1)]

...
def plot_grid(grid_params):
    ...
    plt.hlines(z, xmin=0., xmax=1., linestyle='--')
    plt.hlines(z_m, xmin=0., xmax=1.)

    for layer in z:
        plt.text(0.1, layer, str(layer) + ' m', va='top')

    plt.title(r'Grid with $\Delta z_1 = {}$ m, $b = {}$ and $nz = {}$'.\
              format(grid_params['z1'], grid_params['b'],\
                     grid_params['nz']))
...
def plot_jacobian():
    ...
    plt.plot(z_m, metric_zmid, linestyle='', marker='x',\
             label=r'$\frac{\partial \xi}{\partial z}$ at Full Layers')
    plt.plot(z, metric_zhalf, linestyle='', marker='^',\
...
def plot_conductivities(plant_params, hydraulic_params, hydraulic_params_mvg):
    ...
    from fluxes import hydraulics_rjitema, hydraulics_mvg
    ...
    theta_m = np.linspace(0., theta_max_m, 10000)
    ...
    k_rij, d_rij = hydraulics_rjitema(theta, 1., k0_rij,\
                                  k_rij, d_rij, hydraulic_params)
    k_mvg, d_mvg = hydraulics_mvg(theta_m, 1., k0_mvg,\
                                  k_mvg, d_mvg, hydraulic_params_mvg)
    ...
    plt.plot(theta_interpol, k_rij, label=r'$K_\mathrm{RIJTEMA}$', color='red', alpha=0.5, linestyle='--')
    plt.plot(theta_interpol, d_rij, label=r'$D_\mathrm{RIJTEMA}$', color='blue', alpha=0.5, linestyle='--')
...
def plot_saturation(output, out_ds):
    if p.options['hydparam'] == 'rjitema':
        output.update({'sat':\
                       (output['w_vol'] / p.hydraulic_params['pore_volume'])})
    elif p.options['hydparam'] == 'mvg':
        output.update({'sat':\
                       (output['w_vol'] / p.hydraulic_params_mvg['pore_volume'])})

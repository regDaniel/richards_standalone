...
def hydraulics_rjitema(w_vol, dz, k0, k, d, hydraulic_params):
    ...
    if not (len(k0) == nz+1 or len(k0) == 1):
        raise RuntimeError('k0 must either be an array of length nz+1 \
                            or a scalar')
    ...
    d[0] = float_wp(0.) #boundary condition
...
def hydraulics_mvg(w_vol, dz, k0, k, d, hydraulic_params_mvg):
    ...
    k[j] = k0[j] * np.sqrt(rh[j]) * (float_wp(1.) - \
           np.abs((1.) - (np.abs(rh[j]))**(float_wp(1.)/m_mvg))**m_mvg )**float_wp(2.)
    d[j] = (float_wp(1.) - m_mvg) * k[j] / (m_mvg * alp_mvg) * \
            float_wp(1.) / (w_vol_interpol[j] - adp) * \
            (np.abs(rh[j]))**(-float_wp(1.)/m_mvg) * \
            (np.abs((np.abs(rh[j]))**(-float_wp(1.)/m_mvg) - float_wp(1.)))**(-m_mvg)

    #limit diffusion, in case of NaN it will drop to max value.
    diffmax = float_wp(10.*5.31E-6)
...
def est_flux_fg(k, d, dflux_fg, w_vol, dz_h):
    ...
    dflux_fg[jp1] = - d[jp1] * float_wp(1.)\
                    / dz_h[jp1] * (w_vol[jp1] - w_vol[j])
    kflux_fg      = k

    dflux_fg[-1] = float_wp(0.)
    kflux_fg[-1] = float_wp(0.) #boundary condition
...
def est_qground_fg(qground, w_vol, dz, z, k0_m, runoff_params, hydraulic_params):
    ...
    if j_sat > 0:
        d1 = min(float_wp(1.), max(float_wp(0.),\
             (w_vol[j_unsat] - w_vol[j_unsat-1])) / \
...
def flux_corr(k_flux, d_flux, d, infil, qground, w_vol, dz, dz_h,\
              time_params, hydraulic_params, runoff_params):
    ...
    for j in range(nz-1, -1, -1):
        alpha_out = max(float_wp(0.), min(float_wp(1.), (w_vol[j] - air_dry_pt) / (dt * \
                                (k_flux[j+1]/dz[j] + \
                                 qground[j]))))
    ...
    for j in range(nz-1, -1, -1):
        k_limit = (k_flux[j+1] + \
                  dz[j]*(( pore_volume - w_vol[j])/dt + qground[j]))
        k_flux[j] = max(float_wp(0.), min(k_flux[j], k_limit))

    #surface runoff
    qsurf = max(float_wp(0.), infil - k_flux[0])
    k_flux[0] = min(k_flux[0], infil)

    return k_flux, d, qground, qsurf


def flx_corr_full(k_flux, d_flux, d, infil, qground, w_vol, dz,  dz_h,\
              time_params, hydraulic_params, runoff_params):
    ...
    #overdepletion
    for j in range(nz-1, -1, -1):
        flux_out = k_flux[j+1] + max(float_wp(0.), d_flux[j+1]) - min(float_wp(0.), d_flux[j])
        alpha_out = max(float_wp(0.), min(float_wp(1.), (w_vol[j] - air_dry_pt) / (dt * \
                                (flux_out/dz[j] + \
                                 qground[j]) + eps)))
        k_flux[j+1] = k_flux[j+1] * alpha_out
    ...
    for j in range(nz-1, -1, -1):
        ...
        k_limit = (flux_out2 + \
                  dz[j]*(( pore_volume - w_vol[j])/dt + qground[j]))
        k_flux[j] = max(float_wp(0.), min(k_flux[j], k_limit+eps))
...
def flux_corr_transformed(k_flux, d_flux, d, infil, qground_tr, w_vol_tr, dzeta,\
              time_params, hydraulic_params, runoff_params, metric_zmid):
    ...
    #overdepletion
    for j in range(nz-1, -1, -1):
        alpha_out = max(float_wp(0.), min(float_wp(1.), (w_vol_tr[j] - air_dry_pt_tr[j]) / (dt * \
                                (k_flux[j+1]/dzeta + \
                                 qground_tr[j] + eps))))
        k_flux[j+1] = k_flux[j+1] * alpha_out
    ...
    for j in range(nz-1, -1, -1):
        k_limit = (k_flux[j+1] + \
                  dzeta*(( pore_volume_tr[j] - w_vol_tr[j])/dt + qground_tr[j]))
        k_flux[j] = max(float_wp(0.), min(k_flux[j], k_limit))
...
def overflow_clip(w_vol, qground, hydraulic_params, time_params):
    ...
    qground = qground + np.maximum(float_wp(0.), w_vol - pore_volume) / time_params['dt']

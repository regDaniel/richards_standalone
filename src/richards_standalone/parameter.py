...
hydraulic_params = {                                                  \
'k0'                : float_wp(531.*10**(-8)),                        \
'k1'                : float_wp(-19.88),                               \
'd0'                : float_wp(357.*10**(-8)),                        \
'd1'                : float_wp(-7.44),                                \
...
def update_parameter(update_dict):
    ...
    for u_key in update_dict.keys():
        if u_key in time_params.keys():
            ...
            raise RuntimeError('Parameter key "{}" could not be found, please\
            check spelling'.format(u_key))
